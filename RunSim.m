...
CS=7*A+[1:A]; % cumulative severe infections
...
box off;
set(gca,'LineWidth',2,'tickdir','out','XTick',[1.1:0.01:1.2],'Xminortick','on','Fontsize',26,'YminorTick','on');
xlim([1.1 1.2]);
legend({'Ages 0-19','Ages 20-49','Ages 50-64','Ages 65+'},'location','NorthWest','Fontsize',26);
legend boxoff;
ax = gca;
...
print(gcf,['Figure1C.png'],'-dpng','-r300');
figure('units','normalized','outerposition',[0 0 1 1]);
bb=area(R0v,Deaths'./1000,'LineStyle','none');
for ii=1:4
 bb(ii).FaceColor=Colr(ii,:);
...
print(gcf,['Figure1D.png'],'-dpng','-r300');